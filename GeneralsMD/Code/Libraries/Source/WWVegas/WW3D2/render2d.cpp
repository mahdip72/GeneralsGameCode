...
void Render2DClass::Render()
{
	...
	// The viewport is published through the neutral renderer contract.
	rts::render::SetGameViewport(0,0,static_cast<unsigned int>(width),
	...
	DynamicVBAccessClass vb(rts::render::GAME_BUFFER_TYPE_DYNAMIC_IMMEDIATE,dynamic_fvf_type,Vertices.Count());
	{
		...
		for (i=0; i<Vertices.Count(); i++)
		{
			...
			*(Vector2*)(va+fi.Get_Tex_Offset(1))=Vector2(0.0f,0.0f);
			va+=fi.Get_FVF_Size();
		}
	}
