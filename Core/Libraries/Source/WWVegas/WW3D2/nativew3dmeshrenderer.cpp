...
// N-patch support is queried from the active renderer owner.  This keeps the
// serialized mesh path independent of any graphics SDK declaration.
static bool Supports_NPatch_Geometry()
{
	return rts::render::GameRendererSupportsNPatches();
}
...
DX8FVFCategoryContainer::DX8FVFCategoryContainer(unsigned FVF_,bool sorting_)
	...
	AnyDelayedPassesToRender(false)
{
	if ((FVF&DX8_FVF_TEX1)==DX8_FVF_TEX1) uv_coordinate_channels=1;
	if ((FVF&DX8_FVF_TEX2)==DX8_FVF_TEX2) uv_coordinate_channels=2;
	if ((FVF&DX8_FVF_TEX3)==DX8_FVF_TEX3) uv_coordinate_channels=3;
	if ((FVF&DX8_FVF_TEX4)==DX8_FVF_TEX4) uv_coordinate_channels=4;
	if ((FVF&DX8_FVF_TEX5)==DX8_FVF_TEX5) uv_coordinate_channels=5;
	if ((FVF&DX8_FVF_TEX6)==DX8_FVF_TEX6) uv_coordinate_channels=6;
	if ((FVF&DX8_FVF_TEX7)==DX8_FVF_TEX7) uv_coordinate_channels=7;
	if ((FVF&DX8_FVF_TEX8)==DX8_FVF_TEX8) uv_coordinate_channels=8;
}
...
unsigned DX8FVFCategoryContainer::Define_FVF(MeshModelClass* mmc,bool enable_lighting)
{
	...
	case 0:
		break;
	case 1: fvf|=DX8_FVF_TEX1; break;
	case 2: fvf|=DX8_FVF_TEX2; break;
	case 3: fvf|=DX8_FVF_TEX3; break;
	case 4: fvf|=DX8_FVF_TEX4; break;
	case 5: fvf|=DX8_FVF_TEX5; break;
	case 6: fvf|=DX8_FVF_TEX6; break;
	case 7: fvf|=DX8_FVF_TEX7; break;
	case 8: fvf|=DX8_FVF_TEX8; break;
	}
	...
	fvf|=DX8_FVF_NORMAL;	// Realtime-lit
	return fvf;
}
...
void DX8RigidFVFCategoryContainer::Add_Mesh(MeshModelClass* mmc_)
{
	...
	*/
	...
	for (i=0; i<split_table.Get_Vertex_Count(); i++)
	{
		*(Vector3*)(vb+fi.Get_Location_Offset())=locs[i];
		...
		if ((FVF&DX8_FVF_NORMAL)==DX8_FVF_NORMAL && norms) {
			*(Vector3*)(vb+fi.Get_Normal_Offset())=norms[i];
		}

		if ((FVF&DX8_FVF_DIFFUSE)==DX8_FVF_DIFFUSE) {
			if (diffuse) {
		...
		if ((FVF&DX8_FVF_SPECULAR)==DX8_FVF_SPECULAR) {
			if (specular) {
	...
	*/
	int uvcount = 0;
	if ((FVF&DX8_FVF_TEX1) == DX8_FVF_TEX1) {
		uvcount = 1;
	}
	if ((FVF&DX8_FVF_TEX2) == DX8_FVF_TEX2) {
		uvcount = 2;
	}
	if ((FVF&DX8_FVF_TEX3) == DX8_FVF_TEX3) {
		uvcount = 3;
	}
	if ((FVF&DX8_FVF_TEX4) == DX8_FVF_TEX4) {
		uvcount = 4;
	}
	if ((FVF&DX8_FVF_TEX5) == DX8_FVF_TEX5) {
		uvcount = 5;
	}
	if ((FVF&DX8_FVF_TEX6) == DX8_FVF_TEX6) {
		uvcount = 6;
	}
	if ((FVF&DX8_FVF_TEX7) == DX8_FVF_TEX7) {
		uvcount = 7;
	}
	if ((FVF&DX8_FVF_TEX8) == DX8_FVF_TEX8) {
		uvcount = 8;
	}
...
void DX8TextureCategoryClass::Render()
{
	...
	rts::render::SetGameShader(theShader);
	...
	if (m_gForceMultiply && theShader.Get_Dst_Blend_Func() == ShaderClass::DSTBLEND_ZERO) {
		...
		rts::render::SetGameRenderState(rts::render::GAME_RENDER_STATE_SOURCE_BLEND, rts::render::RENDER_BLEND_DESTINATION_COLOR);
	}
	...
	while (prt) {
		...
		if (WW3D::Expose_Prelit()) {
			switch (mesh->Peek_Model()->Get_Flag (MeshGeometryClass::PRELIT_MASK)) {
				...
				case MeshGeometryClass::PRELIT_LIGHTMAP_MULTI_TEXTURE:
					...
					rts::render::SetGameTexture(0, Peek_Texture (0));
		...
		*/
		...
		if (lenv != nullptr) {
			...
			rts::render::SetGameLightEnvironment(lenv);
		}
		else {
		...
		*/
		...
		if (identity) {
			...
			{ Matrix3D identity; identity.Make_Identity(); rts::render::SetGameTransform(rts::render::GAME_TRANSFORM_WORLD, identity); };
		}
		else {
			SNAPSHOT_SAY(("Set_World_Transform"));
			rts::render::SetGameTransform(rts::render::GAME_TRANSFORM_WORLD, *world_transform);
		}

		if (mesh->Get_ObjectScale() != 1.0f)
			rts::render::SetGameRenderState(
				rts::render::GAME_RENDER_STATE_NORMALIZE_NORMALS, 1);
		...
		*/
		...
		if ((!!mesh->Peek_Model()->Get_Flag(MeshGeometryClass::SORT)) && WW3D::Is_Sorting_Enabled()) {
			...
			{	//mesh has material override of some kind
				...
				if (mesh->Get_Alpha_Override() != 1.0)
				{
					...
					rts::render::SetGameRenderState(rts::render::GAME_RENDER_STATE_ALPHA_REFERENCE, 0x60);
				...
				rts::render::SetGameMaterial(nullptr);	//force a reset of vertex material since we secretly changed opacity
		...
		if (mesh->Get_ObjectScale() != 1.0f)
			rts::render::SetGameRenderState(
				rts::render::GAME_RENDER_STATE_NORMALIZE_NORMALS, 0);
        }
...
void DX8MeshRendererClass::Flush()
{
	...
	*/
	...
	rts::render::SetGameIndexBuffer(static_cast<const IndexBufferClass *>(nullptr), 0);
}
...
void DX8MeshRendererClass::Render_Decal_Meshes()
{
	...
	rts::render::SetGameRenderState(rts::render::GAME_RENDER_STATE_Z_BIAS, 8);
