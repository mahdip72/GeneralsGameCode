...
//		Vector4 * color = Model->Get_Color_Array(color_array_index);
		...
		color[VertCount]=rts::render::ConvertGameColorClamp(Vector4(r,g,b,a));
//		color[VertCount].X = r;
//		color[VertCount].Y = g;
//		color[VertCount].Z = b;
//		color[VertCount].W = a;
...
//		Vector4 * color_list = Model->Get_Color_Array(color_array_index);
		...
		color_list[index] = rts::render::ConvertGameColorClamp(color);
	}
...
*/
	unsigned * color_list = Model->Get_Color_Array(color_array_index);
	// set the proper color for all the existing vertices
	unsigned vertex_color=rts::render::ConvertGameColorClamp(CurVertexColor[color_array_index]);
	for (int lp = 0; lp < VertCount; lp++) {
