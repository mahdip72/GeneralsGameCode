...
bool GameWebBrowser::Initialize(const char *badPageUrl,
	const char *loadingPageUrl,
	const char *mouseFileName,
	const char *mouseBusyFileName)
{
	return DX8WebBrowser::Initialize(badPageUrl, loadingPageUrl, mouseFileName,
		mouseBusyFileName);
}
...
void GameWebBrowser::Render(int backBufferIndex)
{
	DX8WebBrowser::Render(backBufferIndex);
}

void GameWebBrowser::CreateBrowser(const char *browserName,
	const char *url,
	int x,
	int y,
	int width,
	...
	void *gameDispatch)
{
	DX8WebBrowser::CreateBrowser(browserName, url, x, y, width, height, updateTicks,
		static_cast<LONG>(options), reinterpret_cast<LPDISPATCH>(gameDispatch));
}

void GameWebBrowser::DestroyBrowser(const char *browserName)
{
	DX8WebBrowser::DestroyBrowser(browserName);
}

bool GameWebBrowser::IsBrowserOpen(const char *browserName)
{
	return DX8WebBrowser::Is_Browser_Open(browserName);
}

void GameWebBrowser::Navigate(const char *browserName, const char *url)
{
	DX8WebBrowser::Navigate(browserName, url);
}
