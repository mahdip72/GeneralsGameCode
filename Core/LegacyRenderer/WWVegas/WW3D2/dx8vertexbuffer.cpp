...
{
	return buffer != 0 && ::Publish_Render_Buffer_Change(buffer, binding, data,
	...
	rts::render::GpuHandle *validated) const
{
	if (validated == nullptr)
		return false;
	*validated = rts::render::GpuHandle();
	if (Type != BUFFER_TYPE_DYNAMIC_DX8 || VertexBuffer == nullptr ||
...
{
	return VertexBuffer == nullptr ? 0U :
...
{
	return VertexBuffer == nullptr ? 0U : VertexBuffer->FVF_Info().Get_FVF_Size();
}

const void *DynamicVBAccessClass::Get_Sorted_Vertex_Data() const
{
	if (Type != BUFFER_TYPE_DYNAMIC_SORTING || VertexBuffer == nullptr)
		return nullptr;
