...
// Keep the historical source name for the standalone WorldBuilder build, but
// make it an exact alias of the neutral two-slot interface.  DX8Wrapper's
// stored pointer and setter below use GameRenderCleanupHook directly; this
// alias introduces no adapter object or vtable change.
typedef rts::render::GameRenderCleanupHook DX8_CleanupHook;
...
public:
	...
	static void SetCleanupHook(
		rts::render::GameRenderCleanupHook *pCleanupHook)
	{
		m_pCleanupHook = pCleanupHook;
	};
	/*
...
protected:
	...
	*/
	...
	// The neutral renderer global is canonical.  Keep IsWindowed as the
	// historical ABI mirror for x86 callers that still resolve the symbol.
	static bool Is_Windowed() { return rts::render::GameRenderer_IsWindowed; }
	...
	*/
	...
	static rts::render::GameRenderCleanupHook *m_pCleanupHook;
...
WWINLINE void DX8Wrapper::Set_Ambient(const Vector3& color)
{
	...
	rts::render::TrackLegacyGlobalAmbient(
		rts::render::DecodeLegacyAmbientColor(packed_color));
}
