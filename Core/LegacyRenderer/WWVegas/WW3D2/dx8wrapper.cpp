...
// The external Win32 renderer owns the neutral process-wide presentation
// settings when the native renderer is not part of the target.  Keep this
// storage in the same adapter that consumes it so x86 and x64 use one
// renderer-facing contract without a second, divergent set of knobs.
namespace rts {
namespace render {
...
namespace
{
// Keep the opaque RenderDeviceDescClass payload layout identical to the
// legacy pair of fields without importing D3D8 into rddesc.h.  The adapter
// computes the identifier offset from the compiler's actual packing (x86 is
// 4-byte aligned; x64 is 8-byte aligned), rather than assuming that
// sizeof(D3DCAPS8) is always a valid offset.
struct LegacyRenderDevicePayloadLayout
{
	D3DCAPS8 Caps;
...
enum
{
	LEGACY_RENDER_DEVICE_IDENTIFIER_OFFSET =
		offsetof(LegacyRenderDevicePayloadLayout, Identifier),
	LEGACY_RENDER_DEVICE_REQUIRED_STORAGE_BYTES =
		LEGACY_RENDER_DEVICE_IDENTIFIER_OFFSET +
		sizeof(D3DADAPTER_IDENTIFIER8)
};

// This typedef is the VC6-compatible equivalent of static_assert.  It
// intentionally fails the adapter build if a future SDK grows either
// structure beyond the neutral storage budget.
typedef char LegacyRenderDeviceStorageSizeCheck[
	LEGACY_RENDER_DEVICE_REQUIRED_STORAGE_BYTES <=
	LEGACY_RENDER_DEVICE_STORAGE_BYTES ? 1 : -1];

bool Legacy_Render_Device_Storage_Is_Usable(
	const RenderDeviceDescClass &description)
{
	const void *storage = description.Get_Legacy_Data();
	if (storage == 0 ||
		LEGACY_RENDER_DEVICE_REQUIRED_STORAGE_BYTES >
		LEGACY_RENDER_DEVICE_STORAGE_BYTES)
	{
		return false;
	}
	// LegacyRenderDeviceStorage's double member guarantees at least the
	// alignment required by the D3D8 structures on both supported ABIs.
	return (reinterpret_cast<size_t>(storage) % sizeof(double)) == 0;
}

D3DCAPS8 &Legacy_Render_Device_Caps(RenderDeviceDescClass &description)
{
	return *reinterpret_cast<D3DCAPS8 *>(description.Get_Legacy_Data());
}

const D3DCAPS8 &Legacy_Render_Device_Caps(
	const RenderDeviceDescClass &description)
{
	return *reinterpret_cast<const D3DCAPS8 *>(
		description.Get_Legacy_Data());
}

D3DADAPTER_IDENTIFIER8 &Legacy_Render_Device_Identifier(
	RenderDeviceDescClass &description)
{
	unsigned char *storage = static_cast<unsigned char *>(
		description.Get_Legacy_Data());
	return *reinterpret_cast<D3DADAPTER_IDENTIFIER8 *>(
		storage + LEGACY_RENDER_DEVICE_IDENTIFIER_OFFSET);
}

const D3DADAPTER_IDENTIFIER8 &Legacy_Render_Device_Identifier(
	const RenderDeviceDescClass &description)
{
	const unsigned char *storage = static_cast<const unsigned char *>(
		description.Get_Legacy_Data());
	return *reinterpret_cast<const D3DADAPTER_IDENTIFIER8 *>(
		storage + LEGACY_RENDER_DEVICE_IDENTIFIER_OFFSET);
}
...
bool Shutdown_Native_Product_Device_Result(void *)
{
	return _D3D11Bridge.Shutdown_Result() ==
		rts::render::RENDER_RESULT_OK;
}
...
bool Bind_Native_Product_Device_Lifecycle()
{
	...
	operations.shutdownResult = Shutdown_Native_Product_Device_Result;
...
rts::render::GameRenderCleanupHook *DX8Wrapper::m_pCleanupHook=nullptr;
	...
	*/
	...
	if (!lite) {
		...
		if (_RenderDeviceDescriptionTable.Count() == 0)
		{
			...
			{
				WWDEBUG_SAY(("Native product device teardown is pending; retaining lifecycle ownership."));
				return false;
			}
			IsInitted = false;
			return false;
...
void DX8Wrapper::Shutdown()
{
	...
	IsDeviceLost = false;
	// Command-line/headless setup may have changed the neutral value before
	// initialization.  Reconcile the retained ABI mirror before teardown so
	// no legacy caller observes a second windowed-state value after shutdown.
	IsWindowed = rts::render::GameRenderer_IsWindowed;
	if (_UseD3D11Backend)
	{
		if (D3DDevice
		...
		if (!_NativeProductDeviceLifecycle.shutdown())
		{
			WWDEBUG_SAY(("Native product device teardown failed; retaining DX8Wrapper state for owner-thread retry."));
			return;
		}
#else
		const rts::render::RenderResult bridge_shutdown_result =
			_D3D11Bridge.Shutdown_Result();
		if (bridge_shutdown_result != rts::render::RENDER_RESULT_OK)
		{
			WWDEBUG_SAY(("D3D11 bridge teardown failed (%d); retaining DX8Wrapper state for owner-thread retry.",
				static_cast<int>(bridge_shutdown_result)));
			return;
		}
#endif
	}
...
bool DX8Wrapper::Create_Device()
{
		...
		CurRenderDevice < _RenderDeviceDescriptionTable.Count())
	{
		CurrentAdapterIdentifier =
			Legacy_Render_Device_Identifier(
				_RenderDeviceDescriptionTable[CurRenderDevice]);
	}
	...
	if (rts::render::GameRenderer_PreserveFPU)
...
device_initialization_failed:
	...
	if (bridge_initialized || _UseD3D11Backend)
	{
		...
		{
			WWDEBUG_SAY(("Native product device teardown is pending after initialization failure; retaining lifecycle ownership."));
		}
#else
		_D3D11Bridge.Shutdown();
#endif
...
void DX8Wrapper::Release_Device()
{
	if (D3DDevice
		...
		if (D3DDevice == nullptr && CurrentCaps == nullptr)
		{
			// The first teardown attempt may have released all legacy
			// device-dependent subsystems before a native bridge failure.  The
			// retry only needs to finish the still-owned native lifecycle.
			return;
		}
		...
		*/
		// A previous bridge shutdown attempt may already have released the
		// device-dependent subsystem graph while retaining native ownership for
		// retry.  Do not dereference the cleared capability object on that retry.
...
RenderDeviceDescClass desc;
	if (!Legacy_Render_Device_Storage_Is_Usable(desc))
	{
		WWDEBUG_SAY(("Render device descriptor legacy storage is incompatible"));
		return;
	}
	::ZeroMemory(&Legacy_Render_Device_Caps(desc),
		sizeof(Legacy_Render_Device_Caps(desc)));
	::ZeroMemory(&Legacy_Render_Device_Identifier(desc),
		sizeof(Legacy_Render_Device_Identifier(desc)));
DX8Caps native_caps(WW3D_FORMAT_A8R8G8B8);
	Legacy_Render_Device_Caps(desc) = native_caps.Get_DX8_Caps();
	strncpy(Legacy_Render_Device_Identifier(desc).Description,
		"Native D3D11 hardware adapter",
		sizeof(Legacy_Render_Device_Identifier(desc).Description) - 1);
	strncpy(Legacy_Render_Device_Identifier(desc).Driver, "d3d11.dll",
		sizeof(Legacy_Render_Device_Identifier(desc).Driver) - 1);
	desc.set_device_name(Legacy_Render_Device_Identifier(desc).Description);
	desc.set_driver_name(Legacy_Render_Device_Identifier(desc).Driver);
	...
	for (int adapter_index=0; adapter_index<adapter_count; adapter_index++) {
		...
		if (res == D3D_OK) {
			...
			*/
			...
			D3DInterface->GetDeviceCaps(adapter_index,WW3D_DEVTYPE,
				&Legacy_Render_Device_Caps(desc));
			D3DInterface->GetAdapterIdentifier(adapter_index,
				D3DENUM_NO_WHQL_LEVEL,
				&Legacy_Render_Device_Identifier(desc));

			DX8Caps dx8caps(D3DInterface, Legacy_Render_Device_Caps(desc),
				WW3D_FORMAT_UNKNOWN, Legacy_Render_Device_Identifier(desc));
								   ...
								   bool resize_window,bool reset_device, bool restore_assets)
{
	...
	*/
	...
	if (_UseD3D11Backend && !rts::render::GameRenderer_IsWindowed)
	{
		RECT monitor_rect;
		if (Get_D3D11_Monitor_Rect(&monitor_rect))
		{
	...
	IsWindowed = rts::render::GameRenderer_IsWindowed;
	const bool presentation_windowed =
		rts::render::GameRenderer_IsWindowed || _UseD3D11Backend;
	...
	WWDEBUG_SAY(("Attempting Set_Render_Device: name: %s (%s:%s), width: %d, height: %d, windowed: %d",
		...
		ResolutionWidth,ResolutionHeight,
		(rts::render::GameRenderer_IsWindowed ? 1 : 0)));
	...
	if (resize_window || (_UseD3D11Backend &&
		(!rts::render::GameRenderer_IsWindowed ||
			previous_windowed != rts::render::GameRenderer_IsWindowed))) {
...
bool DX8Wrapper::Toggle_Windowed()
{
	...
	if (IsInitted) {
		...
		if (curr_res == -1) {
			...
			return Set_Device_Resolution (-1, -1, -1,
				!rts::render::GameRenderer_IsWindowed, true);
		}
	}
#endif //WW3D_DX8
...
void DX8Wrapper::Get_Device_Resolution(int & set_w,int & set_h,int & set_bits,bool & set_windowed)
{
	...
	set_bits = BitDepth;
	set_windowed = rts::render::GameRenderer_IsWindowed;
}
...
void DX8Wrapper::Get_Render_Target_Resolution(int & set_w,int & set_h,int & set_bits,bool & set_windowed)
{
	...
	if (CurrentRenderTarget != nullptr) {
		...
		set_windowed	= rts::render::GameRenderer_IsWindowed;	// this doesn't really make sense for render targets (shouldn't matter)...
...
bool DX8Wrapper::Registry_Save_Render_Device( const char * sub_key )
{
	...
	return Registry_Save_Render_Device(sub_key, CurRenderDevice,
		ResolutionWidth, ResolutionHeight, BitDepth,
		rts::render::GameRenderer_IsWindowed, TextureBitDepth);
}
...
//buggy Radeon 9700 driver doesn't apply new ambient unless the material also changes.
		...
		rts::render::TrackLegacyGlobalAmbient(
			rts::render::DecodeLegacyAmbientColor(color));
