	...
	rts::render::GpuHandle *validated) const
{
	if (validated == nullptr)
		return false;
	*validated = rts::render::GpuHandle();
	if (Type != BUFFER_TYPE_DYNAMIC_DX8 || IndexBuffer == nullptr ||
...
{
	return static_cast<unsigned int>(IndexBufferOffset) * sizeof(unsigned short);
}

const unsigned short *DynamicIBAccessClass::Get_Sorted_Index_Data() const
{
	if (Type != BUFFER_TYPE_DYNAMIC_SORTING || IndexBuffer == nullptr)
		return nullptr;
