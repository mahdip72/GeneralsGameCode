...
#include "../../Generals/WW3D2/DDSFileLegacy.h"
#elif defined(RTS_ZEROHOUR)
#include "../../GeneralsMD/WW3D2/DDSFileLegacy.h"
#endif
#endif
	...
	WW3DFormat dest_format)
{
	...
	for (unsigned level=0;level<mips;++level) {
		...
		Legacy_DDS_Copy_Level_To_Surface(dds_file, level, d3d_surface);
		d3d_surface->Release();
	}
	return d3d_texture;
#endif
...
IDirect3DTexture8* TextureLoader::Load_Thumbnail(const StringClass& filename, const Vector3& hsv_shift)//,WW3DFormat texture_format)
{
	...
	if (!thumb) {
		return static_cast<IDirect3DTexture8*>(MissingTexture::_Get_Missing_Texture());
	}
	...
	bool allow_compression)
{
	...
	Targa targa;
	if (TARGA_ERROR_HANDLER(targa.Open(filename, TGA_READMODE),filename)) return (IDirect3DSurface8*)MissingTexture::_Create_Missing_Surface();
	...
	if (TARGA_ERROR_HANDLER(targa.Load(filename, TGAF_IMAGE, false),filename)) return (IDirect3DSurface8*)MissingTexture::_Create_Missing_Surface();
...
#else
	D3DTexture = (IDirect3DBaseTexture8*)MissingTexture::_Get_Missing_Texture();
	Apply(true);
#endif
}
