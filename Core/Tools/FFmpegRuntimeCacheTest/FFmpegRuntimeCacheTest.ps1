...
# A second consumer in the same configure must retain the exact runtime
# closure even though all imported targets already exist.
find_package(FFMPEG REQUIRED)
if(WIN32)
    list(LENGTH RTS_FFMPEG_RUNTIME_DLLS _runtime_dll_count)
    if(_runtime_dll_count LESS 5)
        message(FATAL_ERROR "Repeated FFmpeg discovery lost the runtime DLL closure")
    endif()
    foreach(_runtime_dll IN LISTS RTS_FFMPEG_RUNTIME_DLLS)
        if(NOT EXISTS "${_runtime_dll}")
            message(FATAL_ERROR "Repeated FFmpeg discovery retained a missing DLL: ${_runtime_dll}")
        endif()
    endforeach()
endif()
if(EXPECT_HINT_PRECEDENCE)
