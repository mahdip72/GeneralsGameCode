...
# CTest supplies a shared H: validation scratch parent. Keep every diagnostic
# fixture below a fresh invocation root so a retained output file or reparse
# point from an earlier run cannot collide with this run. The production
# writer remains fail-closed for an existing output sink; this isolation is
# test-harness-only and does not remove or overwrite prior evidence.
$scratchParent = [IO.Path]::GetFullPath($ScratchRoot).TrimEnd('\')
if (-not $scratchParent.StartsWith('H:\',
        [StringComparison]::OrdinalIgnoreCase)) {
    throw 'Stage 5 native fixture diagnostic scratch must remain on H:.'
}
...
if (($scratchParentItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
    throw "Stage 5 native fixture diagnostic scratch parent is a reparse point: $scratchParent"
}
$runRoot = Join-Path $scratchParent ('native-fixture-diagnostic-{0}-{1}' -f
...
if (($runRootItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
    throw "Stage 5 native fixture diagnostic run root is a reparse point: $runRoot"
}
$ScratchRoot = $runRoot
