	...
	const rts::ObjectStatusTimerRuntimeMetrics &status)
{
	...
	printf("AI_PLANNING_MANIFEST epoch=%u captured_snapshots=%llu captured_candidates=%llu requested_batches=%llu submitted_jobs=%llu completed_jobs=%llu serial_fallbacks=%llu shadow_matches=%llu shadow_mismatches=%llu validation_failures=%llu committed_batches=%llu parallel_authoritative_commits=%llu rejected_commits=%llu\n",
		(unsigned)SKIRMISH_AI_REPLAY_EPOCH_CURRENT,
...
void GameEngine::init()
{
	...
	try {
#if defined(_WIN64)
		// Native fresh games always use the current deterministic planning
		// behavior. The execution mode selects only the serial, parallel, or
		// shadow implementation of that behavior; explicitly legacy replay
		// markers remain the sole compatibility gate for the retail algorithm.
		SetGeneralsAICanonicalRuntimeEpoch(
			ShouldEnableGeneralsAICanonicalRuntimeEpoch());
#endif
